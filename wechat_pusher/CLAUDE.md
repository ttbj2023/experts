...
对于数据图表，系统会使用 **Gemini + Google Search** 自动查找真实数据：
...
[[CHART:折线图，显示2019-2024年比特币挖矿中可再生能源占比的变化趋势]]
...
**系统会自动**：
1. 使用 Gemini 分析图表需求
2. 通过 Google Search 查找真实数据
3. 验证数据来源和准确性
4. 生成精确的数据图表

### 手动指定数据（可选）

如果你想使用特定数据，可以在占位符中直接提供：
...
1. **封面图**：每篇文章1张，使用AI生成
2. **数据图表**：根据`[[CHART:...]]`占位符数量（找不到数据时不会生成）
...
# 指定输出路径
python -m src.cli.main convert article.md --output output.html
```
...
2. **添加占位符** → 在需要图表/插图的位置添加占位符（系统会自动查找数据）
3. **执行发布** → 运行`python -m src.cli.main publish article.md`
4. **检查结果** → 在微信公众号后台查看草稿
5. **微调发布** → 确认无误后正式发布
...
- **图表数据查找服务**：`src/services/chart_data_finder.py`
