...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 数轴图，从左到右依次标记P(-a)、Q(a)、R、S(1/a)，各点用小黑点表示，数轴上有刻度线
-->
   A. P 
   B. Q 
   C. R 
...
<!-- IMAGE_PLACEHOLDER
type: 技术图纸
description: 圆形古钱币示意图，中间有边长10 mm的正方形孔，圆外标注7 mm（应该是半径相关尺寸）
-->  
...
<!-- IMAGE_PLACEHOLDER
type: 几何示意图
description: 矩形ABCD示意图，标注顶点A、B、C、D及对应直角三角形的位置。
-->
...
<!-- IMAGE_PLACEHOLDER
type: 几何示意图
description: 扇形OAB示意图，标注顶点O、A、B及内部点P的位置。
-->
...
<!-- IMAGE_PLACEHOLDER
type: 条形统计图
description: 跳远成绩的条形统计图，横轴为成绩类别（犯规、一般成绩、优秀成绩），纵轴为次数。甲和乙的数据用不同颜色或标记区分，图中显示犯规类甲有2次，乙有1次；一般成绩类甲有4次，乙有3次；优秀成绩类甲有3次，乙有4次（根据数据计算）
-->  
...
<!-- IMAGE_PLACEHOLDER
type: 几何示意图
description: 平行四边形ABCD，O是对称中心（对角线交点），BC与圆O相切于E点。图中显示两种思路：一种是过O作AD的垂线；另一种是连接EO并延长
选择其中一位同学的想法，完成证明。  
-->
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 平行四边形ABCD内含圆O，E为BC边上的点（圆与BC相切），标注各顶点A、B、C、D及圆心O
-->  

24. (8分) 如图，在长方形电子屏 ABCD 中，AB=8m，AD=5m，一条公益广告画面的动态效果设计如下：动点P从点A出发沿边AB，BC以2m/s的速度向点C运动，随着DP的移动，画面逐渐展开。  
(1) 写出展开的画面面积S（单位：m²）关于点P的运动时间t（单位：s）的函数表达式；  
(2) 当展开的画面面积达到电子屏面积的4时开始播放广告语，播放时间持续3s，求播放结束时展开的画面面积。  
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 长方形ABCD，A在左下角，B在右下角，C在右上角，D在左上角；点P在AB边上移动，形成三角形ADP（阴影部分表示展开的画面）
-->  
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 坐标系中，A在上方，B在南偏东30°方向，AB=40km；P是AB中点；C从A正南方向出发，D从B正东方向出发，且CPD共线
-->  
