...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 数轴图，从左到右依次标记P、Q、R、S四个点，分别对应- a、a、$\frac{1}{a}$ 的位置，其中P在最左侧，然后是Q在-a和a之间，接着是R在a处，最后是S在$\frac{1}{a}$ 处
-->  
   A. P B. Q C. R D. S  
...
<!-- IMAGE_PLACEHOLDER
type: 统计图表
description: 条形统计图，横轴显示成绩类别（犯规、一般成绩、优秀成绩），纵轴显示次数。甲的犯规次数约3次，一般成绩次数约4次？不对根据数据计算后补全；乙的犯规次数1次，一般成绩次数3次，优秀成绩次数5次。
-->  
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 图(1)显示纸杯展开后的扇环结构，左侧有标注A、B点，半径分别为9和6，圆心为O，曲线AD和BC是弧线，ABCD是扇环区域。
-->  
(1) \widehat{AD}的长为___ cm，OB=___ cm。  
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 图(2)显示矩形纸片，长为b，宽为a，内部有两条弧线（对应扇环的上下边缘）。
-->  
记$a×b$表示两边长分别为a，b（a≤b，单位：cm）的矩形纸片的大小。  
