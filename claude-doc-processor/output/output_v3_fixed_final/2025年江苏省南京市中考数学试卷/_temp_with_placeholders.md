...
5. （2分）实数$-a$，$a$，$\frac{1}{a}$在数轴上对应点的位置如图所示. 下列四个点中，表示1的点可能是（ ）  
   <!-- IMAGE_PLACEHOLDER
   type: 示意图
   description: 数轴示意图，从左到右依次标注点P、Q、R、S，分别对应数值-a、a、$\frac{1}{a}$，各点位置用短竖线标记
   -->
   A. P B. Q C. R D. S  
...
<!-- IMAGE_PLACEHOLDER
type: 技术图纸
description: 圆形古钱币示意图，内部有边长10 mm的正方形孔，圆外标注半径相关尺寸7 mm（从圆心到正方形边的距离）
-->
...
<!-- IMAGE_PLACEHOLDER
type: 统计图表
description: 条形统计图，横轴显示成绩类别（犯规、一般成绩、优秀成绩），纵轴显示次数。甲的犯规次数约2次？乙的犯规次数约1次；一般成绩和优秀成绩对应不同高度柱状。
-->
...
<!-- IMAGE_PLACEHOLDER
type: 几何示意图
description: 平行四边形ABCD，O为对称中心，连接相关线段；BC与圆相切于点E。第一个想法是通过点O作AD的垂线；第二个是连接EO并延长。
-->
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 平行四边形ABCD内切圆示意图，点O为圆心，E在BC边上，标注各顶点A、B、C、D及圆心O和切点E的位置
-->
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 长方形电子屏ABCD示意图，其中A在左下角，B在右下角，C在右上角，D在左上角，动点P在AB边上，DP连接形成三角形ADP，标注各边长度AB=8m，AD=5m
-->
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 码头位置示意图，A在上方，B在南偏东30°方向，P为AB中点，甲从A沿正南到C，乙从B沿正东到D，标注角度30°和各点位置关系
-->
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 纸杯展开后的扇环示意图，包含O点，OA和OB为半径，分别标注9cm和6cm，弧AD和BC，ABCD为扇环部分
-->  
