...
5. (2分) 实数$-a$, $a$, $\frac{1}{a}$在数轴上对应点的位置如图所示. 下列四个点中，表示1的点可能是（ ）
   <!-- IMAGE_PLACEHOLDER
   type: 示意图
   description: 数轴图，从左到右依次标记P、Q、R、S，其中P对应-a，Q对应a，R对应$\frac{1}{a}$，各点位置用线段表示
   -->
   A. P B. Q C. R D. S
...
<!-- IMAGE_PLACEHOLDER
type: 条形统计图
description: 跳远成绩条形统计图，横轴为成绩类别（犯规、一般成绩、优秀成绩），纵轴为次数。图中显示甲的犯规次数约3次，一般成绩约4次，优秀成绩约3次；乙的犯规次数约1次，一般成绩约4次，优秀成绩约4次
-->  
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 左侧显示“可以通过点O作AD的垂线……”的文字提示；右侧显示“也可以连接EO并延长……”的文字提示。两个示意图分别对应两种证明思路
-->
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 方向标北东，A在上方，B在南偏东30°位置，连接AB，P是AB中点，甲从A沿正南到C，乙从B沿正东到D，C、P、D共线
-->
...
<!-- IMAGE_PLACEHOLDER
type: 示意图
description: 图(1)，左侧为圆锥侧面展开的扇形，标注了OA=9cm，OB=6cm，弧AD和弧BC；右侧为矩形纸片示意图，标注了a和b边长。具体图形包含两个部分：左边是扇形OAD（半径9），中间有虚线OB=6，右边是矩形ABCD的展开图。
-->  
