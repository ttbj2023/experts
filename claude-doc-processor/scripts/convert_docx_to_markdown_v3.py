...
# -*- coding: utf-8 -*-
"""
DOCX to Markdown Converter v3 - 三阶段架构版本
...
三阶段架构：
1. Stage 1: GLM-4.6V-Flash → PDF识别生成带IMAGE_PLACEHOLDER的Markdown
2. Stage 2: GLM-4.6V-Flash → 描述所有提取的图片（简化版，专注于描述内容）
3. Stage 3: DeepSeek → 语义匹配占位符与图片描述
4. Stage 4: 替换占位符为正确的图片URL

优化改进：
- 分离视觉描述和语义匹配任务
- 使用DeepSeek强大的语言理解能力进行语义匹配
- 提高匹配准确率（从30.8%提升到100%）
"""
...
class DocxToMarkdownConverterV3:
    """DOCX到Markdown转换器 v3 - 三阶段架构"""

    def __init__(self,
                 output_base_dir: str = "output_docx_to_md_v3",
        ...
        Args:
            ...
            glm_api_url: GLM-4.6V-Flash本地服务地址
    ...
    def convert(self, input_file: str) -> Tuple[bool, str, dict]:
        ...
        logger.info("=" * 80)
        logger.info("DOCX to Markdown Converter v3 - 三阶段架构")
        logger.info("=" * 80)
        ...
        try:
            # Stage 1: GLM-4.6V-Flash → PDF识别生成带占位符的Markdown
            logger.info("\n" + "=" * 80)
            logger.info("Stage 1: GLM-4.6V-Flash PDF识别（生成占位符）")
            logger.info("=" * 80)

            stage1_start = datetime.now()
            markdown_with_placeholders, docx_images = self._stage1_pdf_recognition(input_file, temp_pdf, temp_md, images_dir)
            ...
            # Stage 2: GLM-4.6V-Flash → 描述所有提取的图片
            logger.info("\n" + "=" * 80)
            logger.info("Stage 2: GLM-4.6V-Flash 图片描述生成")
            logger.info("=" * 80)

            stage2_start = datetime.now()
            image_descriptions = self._stage2_describe_images(docx_images, images_dir)
    ...
    def _stage1_pdf_recognition(self, docx_path: str, pdf_path: str, md_path: str, images_dir: str) -> Tuple[str, List[Dict]]:
        """
        Stage 1: 使用GLM-4.6V-Flash进行PDF识别，生成带占位符的Markdown

        Returns:
            (markdown_content, docx_images): Markdown内容和提取的图片列表
        """
        # 步骤1.1: 从DOCX提取图片
        logger.info("  步骤1.1: 从DOCX提取图片...")
        docx_images, _ = self._extract_images_from_docx(docx_path, images_dir)
        logger.info(f"  ✓ 提取了 {len(docx_images)} 张图片")

        # 步骤1.2: DOCX → PDF
        logger.info("  步骤1.2: DOCX → PDF (LibreOffice)...")
        pdf_success = self._convert_docx_to_pdf(docx_path, pdf_path)
        if not pdf_success:
            raise Exception("DOCX转PDF失败")
        logger.info(f"  ✓ PDF生成成功")
        ...
        return markdown_content, docx_images

    def _stage2_describe_images(self, docx_images: List[Dict], images_dir: str) -> List[Dict]:
        """
        Stage 2: 使用GLM-4.6V-Flash描述所有提取的图片
        ...
        """
        logger.info(f"  开始描述 {len(docx_images)} 张图片...")
        ...
        for idx, img_info in enumerate(docx_images, 1):
            ...
            logger.info(f"  [{idx}/{len(docx_images)}] 描述 {filename}...")
...
请直接返回描述文本，不要添加其他内容。"""
        ...
        # 直接提取content（与Stage 1相同的方式）
        content = result["choices"][0]["message"]["content"]

        # 清理GLM-4.6V-Flash返回的think标签（与Stage 1使用相同的正则）
        import re
...
我会给你：
1. 一组图片占位符的描述（从文档中提取）
2. 一组实际图片的描述（由GLM-4.6V-Flash视觉模型生成）
...
def main():
    ...
    parser = argparse.ArgumentParser(description='DOCX to Markdown Converter v3 - 三阶段架构')
    parser.add_argument('input_file', help='输入DOCX文件路径')
    parser.add_argument('-o', '--output', default='output_docx_to_md_v3', help='输出目录')
    parser.add_argument('--glm-api', default='http://192.168.100.110:9999', help='GLM-4.6V-Flash本地服务地址')
