...
- ✅ **MathML 公式支持（Word原生）**
  - 使用 `<math xmlns="http://www.w3.org/1998/Math/MathML">` 标签
  - Word 打开后双击即可编辑公式
  - 支持5种LaTeX语法自动转换
  - 行内和块级公式完美支持
...
**支持的表格特性**：
- ✅ **对齐方式** - `:---`（左对齐）、`:-:`（居中）、`---:`（右对齐）
- ✅ **单元格格式化** - 粗体、斜体、删除线、行内代码
- ✅ **表格内公式** - `$x^2$` 自动转换为MathML
- ✅ **表格内图片** - `![alt](path)` 自动base64内嵌
- ✅ **智能识别** - 至少2个`|`符号才识别为表格（避免几何符号误识别）
...
**Word 映射**:
- 使用 MathML 格式：`<math xmlns="http://www.w3.org/1998/Math/MathML">`
- Word 原生支持，双击可进入公式编辑器编辑
- 行内公式：`display="inline"`
- 块级公式：`display="block"`

**支持的环境**：`equation`、`align`、`gather`、`multline`
...
# ❌ 错误示例
\{x^2\}                  # 不要转义
{FORMULA_INLINE:x^2}     # 这是内部占位符，不应出现在用户文档
```

**提示**: 转换器使用MathML格式（Word原生），双击公式即可编辑

---

### Q4: 表格显示错乱

**原因**: Markdown 表格格式不正确或缺少分隔行
...
# ⚠️ 注意
|AB=CD  # 只有1个|符号，会被识别为几何证明符号，不是表格
```

**表格特性**:
- 至少需要2个`|`符号才识别为表格（避免误识别几何符号）
- 支持对齐：`:---`（左）、`:-:`（中）、`---:`（右）
- 支持单元格内粗体、斜体、公式、图片
...
**支持的LaTeX格式选择**:
- **Pandoc/MathJax** (`$...` / `$$...$$`) - 最通用，推荐
- **标准LaTeX** (`\(...\)` / `\[...\]`) - 学术文档常见
- **LaTeX环境** (`\begin{equation}...\end{equation}`) - 复杂多行公式
